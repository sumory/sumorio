 ...
 * 添加一个文件
 */
exports.add_file = function(req, res, next) {
    if (!req.session || !req.session.user) {
 ...
 * 查询文件夹下文件列表
 */
exports.view_folder_files = function(req, res, next) {
    if (!req.session || !req.session.user) {
        res.render('notify/notify', {
    ...
    mysql.queryOne('select * from folder where id = ? and user_id = ?', [folder_id, user_id], function(err, folder){
        if(err){
            res.render('notify/notify', {error : '查找文件夹基本信息出错'});
            return;
        }
        else if(!folder || folder.length <= 0){
            res.render('notify/notify', {error : '该文件夹不存在或您不具备访问该文件夹的权限'});
            return;
        }
        else{
            mysql.query('select * from file where folder_id = ? and user_id =?  order by create_at desc', [ folder_id,user_id ], function(err, files) {
                if (err) {
                ...
                common.get_all_folders(user_id, function(err, folders){
                    res.render('vdisk/folder_files', {
                        files : files || [],
                        folder : folder,
                        folders : folders || [],
...
exports.delete_file = function(req, res, next) {
    ...
    mysql.update('delete from file where id = ? and user_id = ?', [file_id, user_id], function(err, info){
        if(err){
            res.json({flag : 'failed'});
            return;
        }
...
exports.set_file_public = function(req, res, next) {
    ...
    mysql.update('update file set is_public = ? where id = ? and user_id = ?', [1, file_id, user_id], function(err, info){
        if(err){
            res.json({flag : 'failed'});
            return;
        }
...
exports.set_file_private = function(req, res, next) {
    ...
    mysql.update('update file set is_public = ? where id = ? and user_id = ?', [0, file_id, user_id], function(err, info){
        if(err){
            res.json({flag : 'failed'});
            return;
        }
...
exports.view_file = function(req, res, next){
    ...
    mysql.queryOne('select * from file where id = ?', [file_id], function(err, file){
        if(err){
            res.render('notify/notify', {error : '查看文件详细信息出错,该文件可能不存在或已被删除'});
            return;
        }
        else if(file){
            ...
            if(req.session && req.session.user && (req.session.user.id == file.user_id)){//如果为当前文件拥有者, 则显示文件夹目录和设置信息
                common.get_all_folders(req.session.user.id, function(err, folders){
                    if(err){
                        res.render('notify/notify', {error : '查看文件详细信息时查找文件夹目录出错'});
                        return;
                    }
                    else{
                        res.render('vdisk/file', {file : file, folders : folders, isOwner : true});
                        return;
                    }
                }); 
            }
                ...
                if(file.is_public == '公开'){//文件公开
                    res.render('vdisk/file', {file : file, isOwner : false});
                    return;
                }
                else{//文件私有
...
exports.download_file = function(req, res, next){
    ...
    mysql.queryOne('select * from file where id = ?', [file_id], function(err, file){
        if(err){
            res.render('notify/notify', {error : '下载文件出错,该文件可能不存在或已被删除'});
            return;
        }
        else if(file){
            if(file.is_public == 1 || (req.session && req.session.user && (req.session.user.id == file.user_id))){//文件公开;文件私有，只有拥有者可以下载
                mysql.update('update file set down_count = down_count+1 where id = ?', [file.id], function(err,info){
                ...
                res.render('notify/notify', {error : '您不具备下载该文件权限'});
                return;
            }
        }
        else{
            res.render('notify/notify', {error : '您要下载的文件不存在'});
            return;
        }
    });
};
...
exports.change_file_folder = function(req, res, next){
    if (!req.session || !req.session.user) {
        res.json({flag : 'failed', msg : '用户未登录，不得修改文件属性'});
        return;
    }
    var folder_id = req.params.folder_id;
    var file_id = req.params.file_id;
    mysql.update('update file set folder_id = ? where id = ? and user_id = ?', [folder_id, file_id, req.session.user.id], function(err, info){
        if(err){
            res.json({flag : 'failed', msg : '修改属性失败'});
            return;
        }
        else{
            res.json({flag : 'success', msg : '修改成功'});
            return;
        }
    });
...
exports.files = function(req, res, next){
    ...
    async.auto({
        new_files : function(cb) {
            mysql.query('select * from file  where is_public = ? order by id desc limit ?', [1, 8], function(err, new_files){
                if(err){
                    res.render('notify/notify', {error : '查询最新文件列表出错'});
                    return;
                }
                else{
        ...
        hot_files : function(cb) {
            mysql.query('select * from file  where is_public = ? order by down_count desc limit ?', [1, 10], function(err, new_files){
                if(err){
                    res.render('notify/notify', {error : '查询最新文件列表出错'});
                    return;
                }
        ...
        if (err) {
            res.render('notify/notify', { error : '查找文件列表失败'});
            return;
        }
        res.render('files', {
...
exports.user_share_files = function(req, res, next){
    ...
    mysql.query('select * from file where user_id = ? and is_public = ?', [user_id, 1], function(err, files){
        if(err){
            res.render('notify/notify', {error : '查找用户共享文件发生错误'});
            return;
        }
        else{
            res.render('vdisk/share_files', {user_id : user_id, files : files});
            return;
        }
    });
};
...
function file_user_info(files, func){
    async.map(files, function(file, callback) {   
        mysql.queryOne('select * from user where id = ?', [file.user_id], function(err, user){
            if(err||!user){
                file.user = {};
                callback(null, file);
            }
